...
Weight: ExtraBold (800) - "Bold" (700) didn't feel like enough of a change
for the "much bolder, chunkier" ask, and Black (900) risks the letterforms
blobbing together at 16px. This is a judgment call that can't be verified
without seeing it on the actual screen - easy to regenerate at a different
weight (edit WEIGHT_NAME below) if it needs adjusting either direction.
...
def emit(size, data, sym):
    glyphs = data["glyphs"]
    bidx = data["bitmap_index"]
    out_c = OUT_DIR / f"font_montserrat_extrabold_{size}.c"
    out_h = OUT_DIR / f"font_montserrat_extrabold_{size}.h"
...
def main():
    for size in SIZES:
        ...
        emit(size, data, f"font_montserrat_extrabold_{size}")
