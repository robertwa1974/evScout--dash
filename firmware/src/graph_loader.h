// graph_loader.h - ROUTE.bin paged graph loader with on-demand PSRAM
// cache, ported from jgauchia/IceNav-v3 (lib/router/src/graph_loader.hpp,
...
// Each page holds nodes + edges for one 0.1x0.1 degree cell. At ~1MB free
// PSRAM and ~10-20KB/page average, 48 pages ~= 500-960KB.
static constexpr uint32_t PAGE_CACHE_MAX = 48;

class GraphLoader {
public:
    bool load(uint16_t routeSpeed);
    void unload();
